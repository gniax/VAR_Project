...
public class ObjectivesManager : MonoBehaviour
{
    ...
    public TextMeshProUGUI Objective6TMP;
    public TextMeshProUGUI Objective7TMP;
    public TextMeshProUGUI Objective8TMP;

    private int[] _objectivesStatusConfig = new int[8] { 1, 1, 3, 1, 1, 1, 1, 1 }; // never changes
    ...
    public static ObjectivesManager Instance
    {
        get
        {
            if (_instance == null)
            {
                _instance = FindObjectOfType<ObjectivesManager>();
                if (_instance == null)
                {
                    GameObject obj = new GameObject("ObjectivesManager");
                    _instance = obj.AddComponent<ObjectivesManager>();
                }
            }
            return _instance;
        }
...
#region Objectives-Specific
    ...
    public void ValidateObjective_6()
    {
        if (!_isPlaying || IsValidated(5))
        {
            return;
        }

        if (_objectivesStatusRT[5] - 1 <= 0)
        {
            _objectivesStatusRT[5] = 0;
        }
        else
        {
            _objectivesStatusRT[5]--;
        }
    ...
    public void ValidateObjective_7()
    {
        if (!_isPlaying || IsValidated(6))
        {
            return;
        }

        _objectivesStatusRT[6] = 0;
        UpdateObjectivesGUI();
        CheckWin();
    }

    public void ValidateObjective_8()
    {
        if (!_isPlaying || IsValidated(7))
        {
            return;
        }

        _objectivesStatusRT[7] = 0;
        UpdateObjectivesGUI();
        CheckWin();
    }
    #endregion
    ...
    public void UpdateObjectivesGUI()
    {
        ...
        Objective6TMP.text = (_objectivesStatusConfig[5] - _objectivesStatusRT[5]).ToString();
        Objective7TMP.text = (_objectivesStatusConfig[6] - _objectivesStatusRT[6]).ToString();
        Objective8TMP.text = (_objectivesStatusConfig[7] - _objectivesStatusRT[7]).ToString();
    }
